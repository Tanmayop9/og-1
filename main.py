...
class BotManager:
    ...
    async def setup_bot_commands(self, bot: FloodBot):
        ...
        async def on_message(message):
            if message.channel.id != FLOOD_CHANNEL_ID:
            ...
            try:
                ...
                # Check if the user ID is in the secure.json file
                if user_id in secure_users:
                    embed = discord.Embed(
                        description=f"<@{user_id}> is secured. You can't flood their DMs!",
                        color=discord.Color.red()
                    )
                ...
                flood_message = discord.Embed(
                    description=(
                        f"> You've been flooded by `{message.author.name}`!\n"
                        f"> Reason: `{reason}`\n"
                        f"> Want to avoid floods? Join our server and use .secure\n"
